...
label="Hypothesis1"
meta_table<-meta_table[meta_table$Material %in% c(  "PE",                                              
                                                    "PS"),]
#First provide grouping column
meta_table$Treatment<-as.character(meta_table$Material)
#The colours in the the next instruction match the factors for meta_table$Groups

meta_table$Treatment[meta_table$Treatment %in% c(  "PE",                                              
                                                   "PS")]
meta_table$Treatment<-factor(meta_table$Treatment,c(
  "PE",                                              
  "PS"
))
meta_table$Groups<-as.factor("all")
environmental_covariates<-c("Treatment")
